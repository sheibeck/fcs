...
**Key context:** Live product, paying Stripe subscribers. ~259 *active* users / ~1.3k views per 7 days per Google Analytics — but the Cognito user pool holds **12,469 registered users** (~48× GA-active; ~98% dormant), confirming a large-scale migration. This makes shadow-login/lazy-migration mandatory (a forced reset would strand ~12k users) and requires batched bulk import for Auth + batched/ramped Firestore load. DynamoDB holds **19,471 items** total (~1.6 per user) — a modest, low-risk dataset to migrate (minutes-scale batched load, ~13 Auth import batches); the risk is correctness (mapping + billing continuity), not volume. Not all 12,469 accounts are paying subscribers — the Stripe cohort is a smaller subset scoped from Stripe's own customer list (BILL-04), independent of the account/record counts. MIGP-01 (counts) is now answered; Phase 1 remaining work is MIGP-02 (Cognito Identity-Pool→user identity mapping spike) and MIGP-03 (sample real records → field-by-field mapping). "From scratch" means a new codebase but the same product surface and the same migrated data. This greenfield rebuild **supersedes** the earlier incremental two-milestone plan (Vue 2→3 modernization, then Firebase).
